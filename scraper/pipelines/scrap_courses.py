...
from service.programme import get_not_scraped_programme_dates
...
def scrap_courses():
    ...
    for programme_date in programme_dates:
        ...
        for reunion in reunions:
            ...
            with get_session() as session:
                create_reunion(ReunionCreate(id=f"{programme_date}/R{reunion_number}", raw=reunion), session)
            ...
            for course in courses:
                ...
                with get_session() as session:
                    create_course(CourseCreate(id=f"{programme_date}/R{reunion_number}/C{course_number}", raw=course), session)